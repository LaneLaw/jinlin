...
//    };
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        loginViewModel.getUser_info().observe(getViewLifecycleOwner(), new Observer<List<User>>(){
            @Override
            public void onChanged(List<User> userList) {
                Toast.makeText(tt.getContext(), userList.get(0).toString(), 2000).show();
            }
        });
//
...
//        });
        ...
        login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String url_str = "http://192.168.1.107:1337/user/login";
                MyPostJob post = new MyPostJob(tt.getContext());
                post.setMsg(loginViewModel);
                post.execute(url_str, "{\"username\": \"" + account.getText().toString(),"\", \"password\": \"" + password.getText().toString() + "\"}" );
//                play();
